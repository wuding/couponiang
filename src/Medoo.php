...
class Medoo
{
	...
	{
		# print_r([$query, $map, __METHOD__, __LINE__, __FILE__]); exit;
		$this->sql []= $this->generate($query, $map);
		if ($this->debug_mode)
		{
	...
	{
		# print_r([$data, $map, $conjunctor, __METHOD__, __LINE__, __FILE__]);exit;
		$stack = [];
					...
					elseif ($operator === 'REGEXP' || $operator === '!REGEXP')
					{
						...
						*/
						...
						foreach ($value as $index => $item)
						{
							...
							*/
							$like_clauses[] = $column . ($operator === '!REGEXP' ? ' NOT' : '') . ' REGEXP ' . $map_key . 'R' . $index;
							$map[ $map_key . 'R' . $index ] = [$item, PDO::PARAM_STR];
						}
	...
	{
		# print_r([$where, $map, __METHOD__, __LINE__, __FILE__]);exit;
		$where_clause = '';
